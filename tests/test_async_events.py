...
class TestAsyncEvents(object):
    ...
    @pytest.mark.asyncio
    async def test_list_events(self, mock_events, mock_async_http_client_with_response):
        mock_async_http_client_with_response(
    ...
    async def test_list_events_returns_metadata(
        self, mock_events, mock_async_http_client_with_response
    ):
        mock_async_http_client_with_response(
            http_client=self.http_client,
            status_code=200,
