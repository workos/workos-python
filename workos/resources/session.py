...
class WorkOSAuthorizedOrganization(WorkOSBaseResource):
    ...
    def construct_from_response(cls, response):
        authorized_organization = super(WorkOSAuthorizedOrganization, cls).construct_from_response(response)

        organization = WorkOSUserOrganization.construct_from_response(response["organization"])
        authorized_organization.organization = organization

        return authorized_organization

    def to_dict(self):
        authorized_organizations_dict = super(WorkOSAuthorizedOrganization, self).to_dict()
...
class WorkOSUnauthorizedOrganization(WorkOSBaseResource):
    ...
    @classmethod
    def construct_from_response(cls, response):
        unauthorized_organization = super(WorkOSUnauthorizedOrganization, cls).construct_from_response(response)
        ...
        for reason in response["reasons"]:
            reason = WorkOSUnauthorizedOrganizationReason.construct_from_response(reason)
            unauthorized_organization.reason.append(reason)

        return unauthorized_organization

    def to_dict(self):
        unauthorized_organizations_dict = super(WorkOSUnauthorizedOrganization, self).to_dict()
...
class WorkOSSession(WorkOSBaseResource):
    ...
    def construct_from_response(cls, response):
        ...
        for authorized_organization in response["authorized_organizations"]:
            authorized_organization = WorkOSAuthorizedOrganization.construct_from_response(authorized_organization)
        ...
        for unauthorized_organization in response["unauthorized_organizations"]:
            unauthorized_organization = WorkOSUnauthorizedOrganization.construct_from_response(unauthorized_organization)
